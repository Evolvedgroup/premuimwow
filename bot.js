...
client.on("message", message => {
 var prefix = "+";
if (message.content === "+help") {
  message.channel.send('**روح شيك خاص** :mailbox_with_mail: ');
const embed = new Discord.RichEmbed()
   .setColor("RANDOM")
   .setDescription(`
==================== Management commands =====================
...
+kick [@mention] [reasno] ➾ لاعطاء شخص كيك من السرفر
+mute [@mention] [time] ➾ لاعطاء شخص ميوت بالدقائق
+unmute [@mention] ➾ لفك الميوت عن شخص
+move [@mention] ➾ لنقل شخص لرومك الصوتي
+mutechannel ➾ لاغلاق الشات
+unmutechannel ➾ لفتح الشات
+delete channelname ➾ لمحو شات او روم معين
+clear nember ➾ لمسح عدد معين من الرسائل
+bot ➾ لمعلومات البوت
+server ➾ لمعلومات السرفر
+id ➾ لمعلومات حسابك
+ping ➾ لمعرفة سرعة استجابة البوت
+avatar ➾ للحصول على صورتك
+setvoice ➾ لصنع عداد الموجودين في الرومات الصوتيه
...
antispam(client, {
  ...
  maxBuffer: 4, // الحد الأقصى المسموح به من الرسائل لإرسالها في الفاصل الزمني قبل الحصول على ميوت.
  interval: 10, // مقدار الوقت قبل حصول باند
  warningMessage: "*** بطل سبام ***",
  roleMessage: "***مبروك على الميوت يا بتاع السبام***",
  roleName: "Muted", // اسم رتبة الميوت
  maxDuplicatesWarning: 2, // عدد الرسايل الي قبل التحذيرات
  maxDuplicatesBan: 4, // عدد الرسايل الي يقدر المستخدم يرسلها قبل الميوت
  time: 10,
});
...
client.on('message', message => {
    ...
    if(message.content.startsWith(prefix + "toggleWelcome")) {
        ...
        if(!welcome[message.guild.id]) welcome[message.guild.id] = {
          ...
          if(welcome[message.guild.id].onoff === 'Off') return [message.channel.send(`**The Welcome Is __𝐎𝐍__ !**`), welcome[message.guild.id].onoff = 'On']
          if(welcome[message.guild.id].by === 'On') return [message.channel.send(`**The Welcome Is __𝐎𝐅𝐅__ !**`), welcome[message.guild.id].onoff = 'Off']
...
let args = message.content.split(" ").slice(1)
if(!args.join(" ")) return message.reply(`${prefix}autorole toggle / +autorole set [ROLE NAME]`)
let state = args[0]
